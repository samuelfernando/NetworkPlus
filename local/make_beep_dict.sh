...
gawk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $dict_out/beep-plain.txt $vocab |\
  egrep -v '<.?s>' > $dict_out/vocab-oov.txt

gawk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $vocab $dict_out/beep-plain.txt |\
...
echo SPN >> $dict_out/silence_phones.txt
...
echo "--- Adding SIL and NOISE to the lexicon ..."
echo -e "!SIL\tSIL" >> $dict_out/lexicon.txt
echo -e "!NOISE\tNOISE" >> $dict_out/lexicon.txt
echo -e "<UNK>\tSPN" >> $dict_out/lexicon.txt
